...
var webpages = ['http://db.fowtcg.us'];
var index = 0;
var entries = [];
...
function handle_page(url) {
		...
		if(status === "success") {
			...
			if(index==0) {
				...
				if(args.length != 2) {
					console.log("Missing set acronym!\n");
					phantom.exit();
				}

				var acr = args[1];
				
				//Basic stones don't appear when their set numbers are queried, so we have to account for that
				if(acr.localeCompare("Magic Stones") == 0) {
					page.evaluate(function(acr) {
					document.getElementsByClassName('btn btn-default btn-sm separate')[1].click();
					document.getElementsByClassName('btn btn-default btn-sm separate')[13].click();
					document.getElementsByClassName('btn btn-default')[1].click();
					}, acr);
				} else {
					page.evaluate(function(acr) {
						document.getElementsByClassName('btn btn-default btn-sm separate')[1].click();
						document.getElementsByClassName('btn btn-default btn-xs separate')[1].click();
						document.getElementById('q').value=acr;
						document.getElementsByClassName('btn btn-default')[1].click();
					}, acr);
				}
				...
				var imgCoords = page.evaluate(function() {
					...
					for(t = 0; t < imgs.length; t++) {
						if(imgs[t].src.search(/cards\/.*\/\d\d\d(j|sr)?\.jpg/) != -1)
...
function formatCard(field) {
	...
	//geniuses entered some of the names werid on their own database, so I've gotta make an awkward adjustment to account for it
	cardEntry[0] = cardEntry[0].replace(/\([a-zA-Z]*\d*\-\d*\)/g, "");
	cardEntry[1] = (field[0]["Code"])?(field[0]["Code"].split("-"))[0] + "\t":"\t";
	cardEntry[2] = (field[0]["Code"])?field[0]["Code"].replace(/\s[a-zA-Z]*/g, "") + "\t":"\t";
	...
	if(cardEntry[3].localeCompare("Ruler\t") == 0) {
		...
		cardEntry[0] = cardEntry[0].substring(0,cardEntry[0].length-1);
		cardEntry[2] = cardEntry[2].substring(0,cardEntry[2].length-1) + "-r";
		cardEntry[7] = "0";
		cardEntry[8] = "0";
		cardEntry[11] = cardEntry[11].substring(0,cardEntry[11].length-1);
		if(jruler[0].localeCompare("\t") != 0) {
			cardEntry[0] += ("//" + jruler[0]);
			cardEntry[2] += ("," + jruler[2].substring(0,jruler[2].length-1) + "-j\t");
			cardEntry[7] += ("//" + jruler[7]);
			cardEntry[8] += ("//" + jruler[8]);
			cardEntry[11] += ("//" + jruler[11]);
		} else {
			cardEntry[0] += "\t";
			cardEntry[2] += "\t";
			cardEntry[7] += "\t";
...
function replaceImages(textIn) {
	...
	text = text.replace(/<img src=\"_images\/icons\/1x1\.gif\" class=\"fdb-icon-(free)?(\d*|[buwgrm])\" alt=\"(free\d*|[buwgrm])\">/g, '{$2}');
	...
	text = text.replace(/<img src=\"_images\/icons\/1x1\.gif\" class=\"fdb-icon-rest\">/g, 'Rest');
